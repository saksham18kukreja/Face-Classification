...
class final_model:
    ...
    def extract_face_webcam(self,frame,bound_box,required_size=(160,160)):
        pixel = asarray(frame)
    ...
    def embed_face_webcam(self,model,face):
    ...
    def predict_face_webcam(self,model,face):
    ...
    def run_model(self,model_classify_face):  
        ...
        while True:
            ...
            bbox = self.classifier.detectMultiScale(frame,1.1,5,minSize = (30,30))
            
            for box in bbox:
                face = self.extract_face_webcam(frame,box)
                embed_face = self.embed_face_webcam(model_embed,face)
                name,probability = self.predict_face_webcam(model_classify_face,embed_face)
